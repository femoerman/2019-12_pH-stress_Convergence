...
fig1 <- ggplot(filter(data.all, pHmedium==4.5), aes(y = log_r0mean, x = log_alfamean, shape=Genotype, colour = origin)) + geom_point(size=3) + theme_light() +
  stat_ellipse(inherit.aes = F, data = filter(data.all, pHmedium==4.5), mapping = aes(x = log_alfamean, y = log_r0mean, group=origin), type="norm") +
  ...
  scale_color_manual(values=c("#2db3f9", "#2d33f9", "#e41a1c"), breaks=unique(data.all$origin), name="Origin", labels=c("NpH", "LpH", "ANC")) +
  theme(strip.background =element_rect(fill="white"))+ ylim(-4, -0.4) + xlim(-17, -13) +
  theme(strip.text = element_text(colour = 'black', size=16))

fig2 <- ggplot(filter(data.all, pHmedium==6.5), aes(y = log_r0mean, x = log_alfamean, shape=Genotype, colour = origin)) + geom_point(size=3) + theme_light() +
  stat_ellipse(inherit.aes = F, data = filter(data.all, pHmedium==6.5), mapping = aes(x = log_alfamean, y = log_r0mean, group=origin), type="norm") +
  theme( axis.title=element_text(size=14), axis.text=element_text(size=12), legend.text=element_text(size=16),legend.title=element_text(size=20), strip.text.x=element_text(20),
         plot.title = element_text(size=20, hjust = 0.5),axis.text.x = element_text(angle=90, hjust=1) )  + xlab("α (mL/(h indiv)) (ln)") + ylab(expression("r"[0]*" (1/h) (ln)")) + 
  scale_color_manual(values=c("#2db3f9", "#2d33f9", "#e41a1c"), breaks=unique(data.all$origin), name="Origin", labels=c("NpH", "LpH", "ANC")) +
  theme(strip.background =element_rect(fill="white"))+ ylim(-4, -0.4) + xlim(-17, -13) +
...
ggarrange(fig1, fig2,  nrow=1, ncol=2, align = "hv", common.legend = T, legend =  "right",
          labels =list("A", "B"))
ggsave(filename = "Fig3B.png", path = "4_results/Figures", width = 255, height = 204, units = "mm", device = "png")

#LpH data
#Separate data for model fitting (by pH of the assay medium)
data.low <- filter(data.all, pHmedium==4.5, pHorigin == 4.5)
data.neutral <- filter(data.all, pHmedium==6.5, pHorigin == 4.5)
...
#Fit Bayesian correlation model for r0 and alpha
{
  ...
  rlow.LpH <- extract.samples(sampleslow)$r
  median(rlow.LpH)
  
  act_covlow.LpH <- extract.samples(sampleslow)$r * extract.samples(sampleslow)$sigma[,1] * extract.samples(sampleslow)$sigma[,2]
  median(act_covlow.LpH)
  ...
  #Fit model at neutral pH
  {
    ...
    rneutral.LpH <- extract.samples(samplesneutral)$r
    median(rneutral.LpH)
    
    act_covneutral.LpH <- extract.samples(samplesneutral)$r * extract.samples(samplesneutral)$sigma[,1] * extract.samples(samplesneutral)$sigma[,2]
    median(act_covneutral.LpH)
  }
}
...
#NpH data
#Separate data for model fitting (by pH of the assay medium, origin == "high-evolved")
data.low <- filter(data.all, pHmedium==4.5, pHorigin == 6.5)
data.neutral <- filter(data.all, pHmedium==6.5, pHorigin == 6.5, origin == "high-evolved")
...
#Fit Bayesian correlation model for r0 and alpha
{
  #Fit model at low pH
  {
    ...
    rlow.NpH <- extract.samples(sampleslow)$r
    median(rlow.NpH)
    
    act_covlow.NpH <- extract.samples(sampleslow)$r * extract.samples(sampleslow)$sigma[,1] * extract.samples(sampleslow)$sigma[,2]
    median(act_covlow.NpH)
  ...
  #Fit model at neutral pH
  {
    ...
    rneutral.NpH <- extract.samples(samplesneutral)$r
    median(rneutral.NpH)
    
    act_covneutral.NpH <- extract.samples(samplesneutral)$r * extract.samples(samplesneutral)$sigma[,1] * extract.samples(samplesneutral)$sigma[,2]
    median(act_covneutral.NpH)
  }
}
...
#Ancestor data
#Separate data for model fitting (by pH of the assay medium)
data.low <- filter(data.all, pHmedium==4.5, pHorigin == 6.5, origin != "high-evolved")
data.neutral <- filter(data.all, pHmedium==6.5, pHorigin == 6.5, origin != "high-evolved")
...
#Fit Bayesian correlation model for r0 and alpha
{
  #Fit model at low pH
  {
    ...
    rlow.ANC <- extract.samples(sampleslow)$r
    median(rlow.ANC)
    
    act_covlow.ANC <- extract.samples(sampleslow)$r * extract.samples(sampleslow)$sigma[,1] * extract.samples(sampleslow)$sigma[,2]
    median(act_covlow.ANC)
  ...
  #Fit model at neutral pH
  {
    ...
    rneutral.ANC <- extract.samples(samplesneutral)$r
    median(rneutral.ANC)
    
    act_covneutral.ANC <- extract.samples(samplesneutral)$r * extract.samples(samplesneutral)$sigma[,1] * extract.samples(samplesneutral)$sigma[,2]
    median(act_covneutral.ANC)
  }
}
