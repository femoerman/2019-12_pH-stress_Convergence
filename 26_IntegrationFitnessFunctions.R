...
#Loop over evolved populations and ancestors, and do the integration of fitness (at low pH for low-evolved and ancestors)
for(act_strain in sort(unique(sumdata$strain))){
  j = j + 1
  act_subset <- which(sumdata$timing=="ancestral" & sumdata$pHmedium==5 & sumdata$strain==act_strain)
  ...
  act_subset <- which(sumdata$timing=="cg" & sumdata$pHorigin==5 & sumdata$pHmedium==5 & sumdata$strain==act_strain)
  ...
  for(i in 1:length(act_subset)){
    ...
    curve(drf,0,1000000,col="red",lwd=2,add=T)
    j <- j +1
    fit_sums[j] <- sum(sapply(hist(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")],breaks=seq(0,max(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")]),len=1000000),plot=F)$mids,drf)*hist(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")],seq(0,max(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")]),len=1000000),plot=F)$counts)
    fit_sums_non_weighted[j] <- sum(sapply(hist(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")],breaks=seq(0,max(evolve$indiv_per_ml[which(evolve$strain==act_strain & evolve$pH=="low")]),len=1000000),plot=F)$mids,drf))
    #fit_sums_non_weighted[j] <- sum(sapply(seq(0,max(evolve$indiv_per_ml[which(evolve$strain==act_strain)]),len=10000),drf))
    fit_sums_cat[j] <- "evol_lpH"
    fit_sums_strain[j] <- act_strain
    fit_sums_r0[j] <- r0
  }
...
points(fit_sums[which(fit_sums_cat=="evol_lpH")]~fit_sums_r0_centered[which(fit_sums_cat=="evol_lpH")], pch=21, col="white",bg="red",cex=1.5)
...
abline(fixef(m1)[[1]]+fixef(m1)[[2]],fixef(m1)[[3]]+fixef(m1)[[4]],col="red")
...
######8) Average predictions
{
  ...
  #Create averaged predictions (disregarding strain effects)
  {
    ...
    data4 <- extract.samples(mB4)
    data2 <- extract.samples(mB2)
    
    #Create function to draw for specific values of centeredr0 and evolved
    weightedpred <- function(i, data, data4, data2, weight5, weight4, weight2, n=32000){
      ...
      pred4 <- data4$a + data4$b1*centeredr0 + data4$b2*evolved
      pred2 <- data2$a + data2$b1*centeredr0
      
      weighted.data <- c(sample(pred5, size = as.integer(weight5*n)), sample(pred4, size = as.integer(weight4*n)), sample(pred2, size = as.integer(weight2*n)))
  ...
  data.low.pred <- matrix(unlist(lapply(1:nrow(data.low), weightedpred, data.low, data4, data2, weight5, weight4, weight2)), ncol=3, byrow=T)
  data.anc.pred <- matrix(unlist(lapply(1:nrow(data.anc), weightedpred, data.anc, data4, data2, weight5, weight4, weight2)), ncol=3, byrow=T)
  ...
  data$category <- ifelse(data$evolved==1, "evol_lpH", "anc")
  ...
  figC <- ggplot(as.data.frame(dataBayes), aes(y=fits,  x=centeredr0, colour=category, shape=Strain)) + 
    geom_point(data = data.selected, inherit.aes=F, mapping = aes(y = fits, x = centeredr0), colour="darkgrey", size=7) +geom_point(size=4) + 
    ...
    scale_color_manual(values=c("#2db3f9", "#e41a1c"), breaks=category, name="Origin", labels=c( "LpH", "ANC")) + 
    scale_fill_manual(values=c("#2db3f9", "#e41a1c"), breaks=category, name="Origin", labels=c("LpH", "ANC")) +
...
######9) Calculate density trgulation functions and plot
{
  #Select good example and calculate drf
  {
    ...
    data.all <- data.all[which(data.all$name %in% c("ancestral 6.5 5 B2086.2 ph_anc_1", "cg 5 5 B2086.2 ph_low_2",
  ...
  data.ex1 <- filter(evolve, strain=="B2086.2" & pH=="low")
  data.ex2 <- filter(evolve, strain=="SB3539" & pH=="low")
  case1 <- predict.data[which(predict.data$name %in% c("ancestral 6.5 5 B2086.2 ph_anc_1", "cg 5 5 B2086.2 ph_low_2")), ]
  case2 <- predict.data[which(predict.data$name %in% c("cg 5 5 SB3539 ph_low_2", "ancestral 6.5 5 SB3539 ph_anc_3")), ]
...
#10) Arrange figures
{
  ...
  ggsave(plot = fig1,filename = "Fig4.png", path = "4_results/Figures", width = 350, height = 250, units = "mm", device = "png")
}
