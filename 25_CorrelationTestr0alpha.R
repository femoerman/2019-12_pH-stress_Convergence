...
#Fit Bayesian correlation model for r0 and alpha
{
  ...
  rlow <- extract(sampleslow)$r
  median(rlow)
  
  act_covlow <- extract(sampleslow)$r * extract(sampleslow)$sigma[,1] * extract(sampleslow)$sigma[,2]
  median(act_covlow)
  ...
  #Fit model at neutral pH
  {
    ...
    rneutral <- extract(samplesneutral)$r
    median(rneutral)
    
    act_covneutral <- extract(samplesneutral)$r * extract(samplesneutral)$sigma[,1] * extract(samplesneutral)$sigma[,2]
